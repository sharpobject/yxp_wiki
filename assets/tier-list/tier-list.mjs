...
try {
  const response = await fetch(new URL(`data-${lang}.json?v=3`, import.meta.url));
  if (!response.ok) throw new Error('catalog unavailable');
  const data = await response.json();
  const fates = new Map(data.fates.map(f => [f.id, f]));
  let {state, repaired} = decode(location.search, data);
  ...
  const pool = () => eligibleIds(data, state.character, state.career);
  ...
  const card = id => {
    ...
    b.title = f.name + '\n' + f.description + '\n' + f.tags.join(' · ');
    const img = createFateArtwork(f, lang);
  ...
  for (const job of data.sideJobs) $('#tl-career').append(new Option(job.name, String(job.id)));
  function syncUrl() {
  ...
  function render() {
    ...
    $('#tl-progress').textContent = `${pool().length - state.rows.pool.length} / ${pool().length} ` + text('ranked', '已排名');
  ...
  function showSelection() {
    ...
    $('#tl-inspector').hidden = panel.hidden = controls.hidden = selected == null;
    for (const b of document.querySelectorAll('.tl-card')) b.setAttribute('aria-pressed', String(Number(b.dataset.id) === selected));
    if (selected == null) return;
    const f = fates.get(selected), img = createFateArtwork(f, lang);
  ...
  $('#tl-character').addEventListener('change', () => {
    ...
    if (state.rows.pool.length < pool().length && !confirm(text('Switch character and start a new ranking? You can Undo this change.', '切换角色并重新排名？此操作可撤销。'))) {$('#tl-character').value = state.character;
    $('#tl-career').value = state.career; return;}
    selected = null; $('#tl-search').value = ''; commit(blank(next.id, eligibleIds(data, next.id, state.career), state.title, state.career));
  });
  $('#tl-career').addEventListener('change', () => {
    const career = Number($('#tl-career').value);
    const next = decode(encode({...state, career}), data).state;
    selected = null; $('#tl-search').value = '';
    commit(next, text('Side job updated. Compatible rankings kept; other side-job fates removed. Undo restores the previous list.', '副职已更新，保留适用排名并移除其他副职仙命。可撤销以恢复。'));
  });
  $('#tl-search').addEventListener('input', render);
  $('#tl-reset').onclick = () => {
    if (state.rows.pool.length === pool().length || confirm(text('Return every fate to Unranked? You can Undo this.', '将所有仙命移回未排名？此操作可撤销。'))) {selected = null; commit(blank(state.character, pool(), state.title, state.career));}
  };
  ...
  $('#tl-share').onclick = async () => {
    syncUrl();
    try {await navigator.clipboard.writeText(location.href); status(text('Link copied — includes your title, character, side job and every fate’s position.', '链接已复制，包含标题、角色、副职及全部仙命的位置。'));}
